          ...
          <el-row :gutter="16">
            <el-col :xs="12" :sm="8" :md="6" v-for="g in goodsList" :key="g.id" style="margin-bottom: 16px;">
              <el-card :body-style="{ padding: '12px' }">
                <div style="height: 120px; display: flex; align-items: center; justify-content: center; background: #f5f7fa; border-radius: 8px; overflow: hidden;">
                  <img v-if="g.image" :src="g.image" alt="商品图" style="max-width: 100%; max-height: 100%; object-fit: cover;" />
...
async function exchange(goods) {
  try {
    await ElMessageBox.confirm(`确认用 ${goods.pointsCost} 积分兑换「${goods.name}」？`, '积分兑换', { type: 'warning' })
